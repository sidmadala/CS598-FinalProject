...
for root, dirs, files in os.walk("../wav/Test"):
    for name in files:
            ...
            modelLine = np.zeros(x.size)


            # Get the polynomial fit lines
            for index, curTime in enumerate(x):

                # Store them and continue
                leftLine[index] = LeftModel(curTime)
                rightLine[index] = RightModel(curTime)
                frontLine[index] = FrontModel(curTime)
                backLine[index] = BackModel(curTime)
                modelLine[index] = testModel(curTime)

            plt.plot(x, leftLine, 'r--', x, rightLine, 'bs', x, frontLine, 'g^',
                    x, backLine, '+', x, modelLine, 'k-')
            plt.legend(["Left", "Right", "Front", "Behind", "Test"])
            plt.xlabel("Time Step")
            plt.ylabel("Value")
            title = ""
            filename = graphsFolder + "/" + str(deg) + "polynomial-all"
            if ("back" in name):
                title += "Behind - "
                filename += "behind_"
            elif ("front" in name):
            ...
            if ("left" in name):
                ...
                title += "Right, "
                filename += "right-"
            title += "Polynomial Fit: " + str(deg) + " degree polynomial"
            plt.title(title)
